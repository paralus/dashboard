...
export const longEm = (input) => {
  if (input === undefined) return "—";
  return R.defaultTo("—", input);
};

export const getKubeAge = (ts) => {
  if (ts) {
    const milliseconds = moment().startOf("m").valueOf() - ts;
    ...
    const slicedStrings = values.map(getString).filter(isSomething).slice(0, 2);
...
export const renderHealthyPodsCount =
  (edgeName) =>
  ({ rowData, cellData }) => {
...
export const useDownloadTextAsFile = (_) => {
  return (textToWrite, fileNameToSaveAs, type) => {
    const textFileAsBlob = new Blob([textToWrite], { type });
