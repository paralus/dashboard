...
export function getFormattedTime(seconds) {
  ...
  const appendTime = (t, l) => (t ? `${t} ${l}${t > 1 ? "s" : ""}, ` : "");

  let timeString = "";
  timeString += appendTime(days, "day");
  timeString += appendTime(hours, "hour");
  timeString += appendTime(minutes, "minute");
  return timeString;
}

export function getNumberInThousands(value) {
  value = Number(value);
  if (value < 1000) return value;
  const thousandthValue = value / 1000;

  return Math.floor(thousandthValue) + "K";
}
